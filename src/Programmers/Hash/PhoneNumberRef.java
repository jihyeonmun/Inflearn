...
해시 방식을 통한 비교는 문자열을 하나의 해시코드 숫자로 만든 뒤 두 숫자를 비교합니다.
일단 기준점이 되는 문자열은 한 번 해시코드로 변환되면 이후 문자열 자체가 필요 없어지고,
비교 대상이 되는 문자열은 해시코드로 먼저 변환한 뒤 기준 문자열의 해시코드와 숫자가
같은지 다른지만 체크하면 됩니다.
* */
public class PhoneNumberRef {
